...
2. ~~**The Steam QR code doesn't refresh when it expires.**~~ — fixed. `SteamAuthManager.startQRLogin` now auto-restarts the QR challenge (capped at 5 auto-restarts) when `pollUntilConfirmed` reports expiry, instead of leaving a dead code with just an error.
3. ~~**"Add App…" in My Apps (Phase 2) does nothing.**~~ — fixed. Console logs showed the resulting `NSOpenPanel` was "ordered front from a non-active application and may order beneath the active application's windows" — Mist wasn't reliably "active" from the window server's point of view at the moment the button fired, so the picker opened invisibly behind Mist's own window. Both the Add and Locate… flows now call `NSApp.activate(ignoringOtherApps: true)` right before presenting.
