...
1. ~~**Epic sign-in doesn't work out of the box.**~~ — mechanically fixed 2026-07-13: `legendary` (the community `legendary-gl` fork; matches the `legendary.gl/epiclogin` domain already used for browser sign-in) is now bundled via a new `tools/legendary.pin` + `build-tools.sh` fetch step, same shape as gbe_fork/DXVK, and `LegendaryLocator` checks the bundled path first. Verified locally: dropping the binary into a dev build's `Resources/tools/` makes the Epic Account card's "legendary isn't installed" error disappear outright. **Not fully verified**: the actual Epic OAuth + legendary auth flow end-to-end needs a real Epic account to test, which this session doesn't have — the mechanical blocker (no binary at all) is gone, but the login flow itself is unconfirmed until tested with a real account.
...
10. **Epic login doesn't work — user-confirmed 2026-07-13, screenshot attached.** Same root cause as item 1 (`legendary` wasn't bundled) — item 1's mechanical fix (bundling the binary) landed the same day, so the "isn't installed" error specifically should be gone in the next release. Still needs a real Epic account to confirm the actual sign-in flow works end-to-end — leaving this item open until that's confirmed.
