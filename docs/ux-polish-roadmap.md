...
1. ~~**Epic sign-in doesn't work out of the box.**~~ — fixed and fully confirmed 2026-07-13. `legendary` (the community `legendary-gl` fork; matches the `legendary.gl/epiclogin` domain already used for browser sign-in) is bundled via `tools/legendary.pin` + a `build-tools.sh` fetch step, same shape as gbe_fork/DXVK; `LegendaryLocator` checks the bundled path first. User confirmed a real Epic OAuth login completed successfully end-to-end ("Logged in as haczibi", 154 owned games listed) — this item is done, not just mechanically unblocked.
...
9. ~~**No in-app "clean slate" reset.**~~ — fixed. Settings → Storage & Engine has a Reset card with two confirmed-destructive actions: "Reset Library & Logins…" (`MistReset.resetLibraryAndLogins` — wipes `drive_c`, registry files, the manifest/session/settings JSON files, and UserDefaults, but keeps the downloaded `wine/` engine) and "Full Reset…" (`MistReset.fullReset` — wipes the whole support directory). Both require confirming an alert spelling out exactly what's removed, then quit the app immediately (it can't keep running with its own prefix pulled out from under it) — relaunch to start fresh.
10. ~~**Epic login doesn't work.**~~ — fixed, same as item 1 (same root cause, `legendary` not bundled). User confirmed a real login succeeded after updating.
14. ~~**Epic install UI in Settings is bad.**~~ — user-flagged 2026-07-13 ("the UI sucks ass"), fair: it was genuinely unstyled pre-Foglight leftover code (plain `GroupBox`/`.headline`, no Fog tokens) asking you to type a cryptic internal Epic app name by hand, plus three hardcoded "Quick Install" buttons (Rocket League/Fortnite/Fall Guys) — a per-game hack, not a real solution. Turned out to be fully redundant: `scanEpicGames()` already builds real `Game` entries (real display name, real `app_name` as `id`) for every owned-but-not-installed Epic title, and `handleInstall` already routes Epic installs through `processManager.epicInstall(appName: game.id)` — so any not-yet-installed Epic game already shows a normal, correct Install button in the Epic sidebar section, exactly like Steam. Deleted the text-entry install flow and Quick Install buttons entirely from Settings; restyled what's left (login status) with proper Fog tokens, and added a one-line pointer to the sidebar's Epic section for actually installing games.
11. ~~**Steam avatar still just shows an initial letter, not a real profile photo.**~~ — fixed. Path (b) panned out: `AchievementRelay` has a new `--persona` mode that calls `SteamFriends.RequestFriendInfo` on the signed-in account's own SteamID and reads the resulting `PersonaStateCallback` (`Name` + `AvatarHash`) — no Web API key needed, same client-protocol session as everything else. The hash resolves through Steam's ordinary `avatars.steamstatic.com/{hash}_full.jpg` CDN. Verified live end-to-end against a real session: relay returned a real persona name + hash, the CDN URL served a real 200 OK JPEG, and the sidebar footer now shows the actual photo (confirmed via screenshot) with the persona name instead of the account login name.
...
13. ~~**"Installed" isn't a top-level sidebar row.**~~ — fixed. A dedicated "Installed" row sits between All Games and Steam, showing installed titles across every source at once (composes with the existing filter chips — e.g. picking Family Shared on top of it still works).
15. ~~**Sidebar footer only showed Steam, not Epic.**~~ — user-requested 2026-07-13. Now shows both services' sign-in status at once — real avatar/persona name for Steam (tappable, opens the account's real public `steamcommunity.com` profile in the in-app browser — profiles are public by default, so this works whether or not the embedded browser has a live Steam web session) and username for Epic (presence-only, not tappable — Epic has no equivalent public-by-username profile page to link to, confirmed with the user before building). Both rows dim rather than disappear when signed out, so "not connected yet" reads clearly instead of the row just vanishing.
