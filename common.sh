#!/bin/bash
# common.sh — shared helpers for Mist launch scripts.
#
...
mist_resolve_wine() {
    ...
    if [[ "${script_dir}" == *".app/Contents/Resources"* ]]; then
        WINE_DIR="${HOME}/Library/Application Support/Mist/wine"
    else
...
mist_resolve_prefix() {
    DEFAULT_PREFIX="${HOME}/Library/Application Support/Mist"
    WINEPREFIX="${WINEPREFIX:-${DEFAULT_PREFIX}}"
}

# Export the base Wine environment shared by every launch path.
# Requires mist_resolve_wine + mist_resolve_prefix to have run first.
mist_export_base_env() {
    export WINEPREFIX
...
mist_require_wine() {
    if [[ ! -x "${WINE_BIN}/wine" ]]; then
        ...
        echo "       Run setup first (open Mist.app or ./setup.sh)." >&2
        exit 1
    fi
}

# Pick the most likely main game .exe under a directory.
# Usage: exe=$(mist_main_exe "<game_dir>" [maxdepth])
