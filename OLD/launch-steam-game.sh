...
# Set MIST_NO_GPTK=1 to force the bundled-Wine path instead.
if [[ $NO_EAC -eq 1 ]]; then
    GPTK_WINE="/Applications/Game Porting Toolkit.app/Contents/Resources/wine/bin/wine64"
    if [[ -x "$GPTK_WINE" && "${MIST_NO_GPTK:-0}" != "1" ]]; then
...
if [[ -z "$GAME_EXE" ]]; then
    MAIN_EXE=$(mist_main_exe "${GAME_DIR}")
    if [[ -n "$MAIN_EXE" ]]; then
